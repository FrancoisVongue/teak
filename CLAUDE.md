...
2. **Ref / Array** — gen-проверяемые ссылки на данные, живущие в регионе. Копируемые, передаются и хранятся свободно. Могут протухнуть (регион умер) → доступ ловится gen-проверкой (abort), как и выход за границы массива.
   - `Ref[T]` — ссылка на **одно** значение. `ref(r, v)` выделить, `get(rf)` прочитать, `set(rf, v)` записать. (Сахар над `Array[T]` из одного элемента — потому имя кричит «ссылка», а машинерия переиспользуется.)
   - `Array[T]` — ссылка на **много** значений (буфер). `array(r, N, init)`, `a[i]`, `a[i] := v`, `len(a)`, `slice(a, lo, hi)`.
