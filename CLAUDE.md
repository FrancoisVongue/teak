...
- **Оптимизация** компилятора — это не ось языка; она просто **лоуэрит то, что ты явно написал**, как можно быстрее, не меняя смысла. Никаких доказательств, никакого «компилятор молча решил за тебя».

**Производительность достигается явностью, а не доказательствами.** Программист прямо говорит, что и где живёт, ортогональными конструкциями: какой регион (`region(N)` / `stack_region(N)`), линейный тип для внешнего ресурса, голый `*T` как escape-hatch. Ничего скрытого, никакой системы доказательств в типах. Мы НЕ Haskell и НЕ Roc — никакого «языка над языком», где компилятор за тебя что-то выводит и оптимизирует невидимо. Ближайший родственник по этой оси — Zig: явный allocator/регион, всё видно в коде. Это прямое следствие второго закона («никакой невидимой магии»), и если эти два принципа конфликтуют — побеждает явность.

Пример как НЕ надо: Swift `@escaping`. Плохой не потому, что компилятор «должен был доказать сам» — а потому, что Swift **размножил тип закрытия на два** (escaping и non-escaping) для одной и той же сущности, вместо того чтобы оставить один закон закрытия + отдельную явную ось «куда положить env». У нас: один тип закрытия, env в регионе; стек vs куча — это **твой явный выбор региона** в коде (`stack_region` vs `region`), а не невидимый escape-анализ. Один закон, одна явная ортогональная ось, ноль доказательств.

Поэтому ортогональность не стоит нам рантайм-перформанса: перформанс берётся из того, что **ты сам написал, где быстро**, а компилятор это честно лоуэрит. Никаких «компилятор за тебя понял».
