...
| **Generic над linear типами** (`Option[Fd]`, `Result[Socket, _]`, любой `[T]` с линейным T) | Каждый линейный ресурс уникален — `Fd` хочет close, `Socket` — shutdown→drain→close, `Region` — slab tear-down. Generic-функция, которой нужно знать «как чистить T», уже лезет внутрь T (constraint «T: Drop») = classes под одеялом, ровно то, что отвергнуто строкой выше. См. `CLAUDE.md §6`: «линейное в дженерик не мешать — дженерик не должен знать, как чистить элемент». `Handle[linear T]` (cascade `drop_Array_T`) — спец-механизм для **одного** контейнера, который компилятор понимает структурно; раздуть его в «общий закон агрегатов с linear-полем» = либо каталог спецслучаев (Option, Result, tuples, user enums каждый отдельно), либо classes. Workaround `fd_wrap`/int — **не workaround, а правильная форма**: per-use injection, не per-type механизм. |
