...
### 0. Память: явные дополнения к региональной модели *(планируется)*

Текущий `region(N)` фиксированного размера, abort на overflow. Этого достаточно для known-bounded scopes (per-frame, per-request), но **не покрывает** user-driven unbounded случаи (пользователь открывает произвольное число окон/документов/сессий — потолок неизвестен). Добавляем явный второй примитив, без размытия существующего:

- **`region(N)`** *(есть)* — фиксированный, abort на overflow. **Safety contract** «я знаю максимум». Подходит для bounded scopes; runaway-аллокация падает с понятной ошибкой, а не тихо ест RAM до OOM.
- **`growable_region()`** *(NEW)* — растёт по мере надобности через `realloc` буфера слота. **Тривиально безопасно** в нашей модели: `Handle` хранит `{slot, off, len, gen}`, никогда не сырой адрес; realloc обновляет `RT[slot].buf`, и все хендлы продолжают работать без замены ссылок. **Исключение** — `as_ptr(h)`: сырой указатель протухает через realloc; это явная escape-hatch-ответственность, документируется. Реализация: ~50 строк в runtime (`realloc` на overflow) + новый AST-узел + парс/типизация (копия `TERegion` с другим runtime-вызовом).

Поверх обоих живёт **`std::gen_arena`** (slot-pool с per-slot generation для event-driven лайфтаймов, см. §1). На fixed — bounded pool; на growable — unbounded по числу одновременно живых, но с reuse слотов не растёт сверх пика.

**Каскадный drop через `linear struct` + `Handle[linear]` — это и есть наш «арена на сущность» паттерн.** Окно/документ/сессия = `linear struct X { region: Region, ... }` + обязательный `drop_X(x) { drop(x.region); 0 }`. Линейные сущности хранятся как `Handle[X]` в app-арене; компилятор автоматически генерит каскадный drop, и убийство app-арены проходит по всем `X`, дропая каждый внутренний регион. **Это не RC** — никаких счётчиков, никаких атомиков, никакой платы на алиасинг; цена каскада = ровно цена `free()`-цепочки в детерминированной точке (конец scope), а не «когда-то на последнем decrement». Полный end-to-end тест этого pattern'а (linear struct с линейным полем + ref-single-cell move + `Handle[linear]` каскад) — **планируется**: малый пример `examples/window_manager.orto` через регрессию обоих бэкендов.
...
**Внутренние оптимизации бэкендов (без user-facing осей):**
- **`restrict` в C-бэкенде для линейных параметров.** В типах у нас уже есть железобетонная инфо «не алиасит» (линейные типы по построению, плюс `Handle` с уникальным slot'ом). Передавать её gcc как `restrict` на параметрах указательного типа — потенциальный бесплатный выигрыш в горячих циклах (gcc сможет переставлять загрузки/сохранения). **Никаких изменений в языке** — чисто внутренняя работа emit.ml. Соответствующего хука в QBE нет, но и большой беды от этого тоже — gcc -O2 на C-пути это и есть наш релиз.

**Низкоуровневые штуки из C-арсенала — как библиотека, не как язык** (добавляем по мере спроса):
- **SIMD-интринсики** (`_mm256_*`, NEON `vld1q_*` и т.д.). У нас — `std::simd` через FFI/`extern` к C-интринсикам, когда реальный код запросит. Без специальных языковых конструкций — обычные `extern fn`.
- **Prefetch-хинт** (`__builtin_prefetch`). У нас — `extern fn prefetch(p: *byte)` в `std::raw`. Опять же без новой языковой оси.
- **Cache-line padding** против false-sharing в concurrent-коде. У нас — обычное `record` с padding-полями или однострочный type-alias (`type CacheLine = struct { _pad: [byte; 64] }` или вроде). Никакой `@align(64)`-аннотации не вводим — выравнивание уже даётся через `aligned_region(N, A)` на стороне аллокации; для embedded-в-структуру случая — явные padding-поля, видно в коде.

Общий принцип: всё, что C даёт `__builtin_*`/интринсиками — это **функции через FFI**, не язык. Ось языка не растёт ни на одну позицию.
