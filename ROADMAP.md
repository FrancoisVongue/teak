...
### 2. QBE-бэкенд *(планируется)*

Заменить связку «emit C → gcc» на **QBE** (крошечный SSA-бэкенд, ~10k строк C; amd64/arm64/riscv64). Зачем: быстрая компиляция (нет второго парса C), нормальный IR вместо C-сорс-импеданса, и **полный C ABI задаром** — вызовы C становятся нативными `call` к символу, переобъявления прототипа нет → **весь класс clash'ей растворяется** (`write`/`putchar`/`strlen`/cimport-импорты libc перестают конфликтовать). Тулчейн проверен: qbe собран, hand-IL `ret 42` и вызов libc `strlen` работают без клеша.

План: фронт (lexer→parser→resolve→lift→check→mono) **переиспользуется целиком**; рантайм (слаб региона, gen-check, malloc/free/memcpy/abort, io_uring) **остаётся C** (маленький `runtime.c`, компилится раз, линкуется), QBE-код зовёт его по C ABI. Новый `emit_qbe.ml` **рядом** с `emit.ml` за флагом `--backend qbe`; C-бэкенд держим зелёным, пока QBE догоняет весь suite, потом переключаем дефолт. Майлстоуны: M1 `fn main(){42}` → M2 арифметика/let/if/while → M3 вызовы+C-рантайм → M4 struct/enum/match → M5 замыкания/async. Глубокая оптимизация (LLVM-уровень) — позже, если бенч потребует.
