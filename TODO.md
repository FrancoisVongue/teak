...
копию-по-значению, идёт либо через `Ref`-ячейку (`pos[0] := ..`,
...
мутабельное состояние. `Ref[T]` И ЕСТЬ мутабельная ссылка orto — просто
...
- orto: `Ref[T]` — **копируемая** (много алиасов!) ручка, безопасность
...
arena indices) — это и есть наш `Ref` = регион + индекс + поколение, но
...
- Несколько живых мутабельных алиасов на одну ячейку — тривиально
  (копируемый `Ref`); в Rust это боль.
